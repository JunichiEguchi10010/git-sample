...
→Server Actionsは、「サーバーで実行される関数」を、クライアント側のUIコードと同じ場所で定義・呼び出せる新しい仕組みです。
これにより、フォーム送信やデータ更新の処理が、驚くほどシンプルになります。

🧠 なぜ生まれたのか？（背景と哲学）
従来のNext.jsでは、フォーム送信やデータ更新には以下のような手順が必要でした：
・クライアント側でfetch()などを使ってAPIを呼び出す
・/api/xxx.tsなどのAPIルートを作成して、サーバー側の処理を書く
・状態管理やエラーハンドリングをクライアントで頑張る
・これって「見た目のコード」と「データ処理のコード」が分断されていて、構造的にも感情的にも不自然」ですよね
・Server Actionsはこの分断をなくし、UIとロジックを一体化させることで、「構造の純度」を高めるアプローチです。
...
export async function handleSubmit(formData: FormData) {
  ...
  console.log(`名前を受け取りました: ${name}`);
  // ここでDB保存などの処理を記述
}
ポイントは 'use server' の一文。これが「この関数はサーバーで実行されるよ」とNext.jsに伝える魔法の言葉です。
...
なんと、action={handleSubmit}と書くだけで、フォーム送信時にサーバー関数が実行されるんです。
APIルートもfetch()も不要！

🎁 メリットまとめ
特徴	             内容
✅ シンプル	    UIとロジックが同じ場所に書ける
🔒 安全	        サーバーで実行されるので、秘密情報も安心
🧩 型安全	      TypeScriptとの相性が抜群
🔄 リアクティブ	 revalidatePath()などでUIを即時更新できる
🧪 テストしやすい	関数単位でテスト可能
...
いいねボタン
商品のフィルター更新
つまり、「ユーザーが何かを送信・変更する」場面にぴったりです。

🧩「フロントサイドでバックエンドを“呼び出せる”」ということです
Server Actionsを使うと、フロントエンドのUIコードの中に、サーバーで実行される処理（バックエンド）を直接書いて、呼び出すことができるようになります。

つまり：
✅ UIとロジックが同じファイルにある（分離しなくていい）
✅ APIルートを作らなくていい（/api/xxx.ts不要）
✅ フォーム送信やボタン操作で、サーバー処理が直接呼び出せる

🧠 例えるなら…
従来は「フロントエンドが電話をかけて、バックエンドが応答する」ような関係でした。
Server Actionsでは、「フロントエンドの中に、電話の中身（処理）が直接書いてある」ような感覚です。見た目と中身が一体化しているんですね。

💡 でも注意点もあります
実際に処理が走るのはサーバー側です（ブラウザではなく）
フロントからはフォーム送信やアクションでトリガーするだけ
JavaScriptのfetch()やaxiosのような明示的な通信は不要

✅ まとめ
項目	        Server Actionsの特徴
定義場所	    フロント側のUIコードと同じ場所に書ける
実行場所	    サーバー側（バックエンド）
呼び出し方法	フォーム送信やaction={関数}で直接
メリット	    UIとロジックの統合、コードの簡潔化、型安全
