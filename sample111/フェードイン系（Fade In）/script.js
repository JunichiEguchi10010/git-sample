...
//   document.addEventListener("DOMContentLoaded", () => {
//     意味：HTMLの構造（DOM）がすべて読み込まれたら、この中の処理を実行します。
    
//     理由：まだ読み込まれていない要素にアクセスするとエラーになるので、DOMが完全にできあがってから処理を始めるためです。
    
//       const sections = document.querySelectorAll(".section");
//     意味：クラス名が「section」のすべての要素を取得して、sections という変数に入れます。
    
//     返り値：複数の要素をまとめたNodeList（配列のようなもの）です。
    
//       const observer = new IntersectionObserver((entries, observer) => {
//     意味：新しく「IntersectionObserver（交差監視）」のインスタンスを作っています。
    
//     目的：特定の要素が画面に表示されたかどうかを監視するための機能です。
...
//             const children = section.querySelectorAll("div");
//     意味：section の中にあるすべての <div> 要素を取得します。
    
//     目的：それら子要素にアニメーションを順番に付けていくための準備です。
    
//             children.forEach((el, index) => {
//     意味：子要素の1つ1つを順番に処理します。
...
//     意味：子要素に対して、先ほど取得した方向名（例：fade-up、fade-left、zoom-in）のクラスを付けます。
    
//     目的：CSSでアニメーションの種類を切り替えるためです。
    
//               el.style.transitionDelay = `${index * 0.2}s`;
//     意味：アニメーションが始まるのを子要素ごとに遅らせます。
...
//     効果：連続的で自然なアニメーションになります。
    
//               el.classList.add("is-visible");
//     意味：子要素に「表示状態」を表すクラスを追加します。
    
//     CSS側でこのクラスに反応して、フェードインなどのアニメーションが実行されます。
    
//             });
//     意味：forEach の終了。すべての子要素への処理が終わりました。
    
//             observer.unobserve(section); // 一度表示したら監視解除
//     意味：このセクションは一度アニメーションしたので、これ以上監視しないようにします。
    
//     理由：パフォーマンスを良くするため。何度もチェックすると無駄が多いからです。
    
//           }
//     意味：if (entry.isIntersecting) の終わり。
    
//         });
//     意味：entries.forEach の終わり。監視対象のすべての変化に対して処理が終わりました。
    
//       }, {
//         threshold: 0.2
//       });
//     意味：IntersectionObserver のオプションです。
